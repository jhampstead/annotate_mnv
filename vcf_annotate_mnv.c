...
int main(int argc, char *argv[]) {
    ...
    int i, j, missing1, missing2;
    int ngt_arr = 0, *gt_arr = NULL, ngt;
    int ngt_arr1 = 0, *gt_arr1 = NULL, ngt1;
    int ngt_arr2 = 0, *gt_arr2 = NULL, ngt2;
    ...
    while(bcf_read(fp_in, header, cr) >= 0) {
		...
		for (i = n_prs-1; i >= 0; i--) {
			if ((cr->rid != prs[i]->rid) || ((cr->pos - mnv_radius) > prs[i]->pos)) {
				...
				ngt1 = bcf_get_genotypes(header, cr, &gt_arr1, &ngt_arr1);
				ngt2 = bcf_get_genotypes(header, prs[i], &gt_arr2, &ngt_arr2);

				for (j = 0; j < nsamples; j++) {
					missing1 = bcf_gt_allele(gt_arr1[j * 2]) + bcf_gt_allele(gt_arr1[j * 2 + 1]);
					missing2 = bcf_gt_allele(gt_arr2[j * 2]) + bcf_gt_allele(gt_arr2[j * 2 + 1]);
